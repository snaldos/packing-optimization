...
class Graph {
 public:
  ...
  Graph(const Graph<T> &other);             // Copy constructor
  Graph &operator=(const Graph<T> &other);  // Copy assignment operator
  /*
 ...
 protected:
  ...
  void clear();  // Helper function to clear the graph
...
template <typename T>
Graph<T>::Graph() : distMatrix(nullptr), pathMatrix(nullptr) {
  // Nothing else to initialize, as vertexSet is an empty vector by default
}
...
Graph<T>::Graph(const Graph<T> &other) {
  ...
  // Deep copy the edges
  for (auto v : other.vertexSet) {
    Vertex<T> *newV = findVertex(v->getInfo());
    for (Edge<T> *edge : v->getAdj()) {
      Vertex<T> *dest = edge->getDest();
      T destInfo = dest->getInfo();
      double weight = edge->getWeight();
      addEdge(newV->getInfo(), destInfo, weight);
    }
  }

  // Deep copy the adjacency matrices
  int n = other.vertexSet.size();

  if (other.distMatrix != nullptr) {
    distMatrix = new double *[n];
    for (int i = 0; i < n; ++i) {
      distMatrix[i] = new double[n];
      std::copy(other.distMatrix[i], other.distMatrix[i] + n, distMatrix[i]);
    }
  }

  if (other.pathMatrix != nullptr) {
    pathMatrix = new int *[n];
    for (int i = 0; i < n; ++i) {
      pathMatrix[i] = new int[n];
      std::copy(other.pathMatrix[i], other.pathMatrix[i] + n, pathMatrix[i]);
    }
  }
}
